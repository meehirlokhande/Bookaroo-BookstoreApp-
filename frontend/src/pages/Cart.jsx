  ...
  const deleteItem = async (bookid) => {
    try {
      const response = await axios.put(
        `/api/v1/remove-book-from-cart/${bookid}`,
        {},
        { headers }
      );
      alert(response.data.message);
      // Update the state to remove the book from the cart
      ...
      {Cart && Cart.length === 0 && (
        <div className='h-screen bg-white rounded'>
          <div className='h-[100%] flex items-center justify-center flex-col'>
            ...
            <img src="https://img.freepik.com/free-vector/supermarket-shopping-cart-concept-illustration_114360-22408.jpg" alt="empty-cart" className='h-[30vh] lg:h-[50vh] mt-8 lg:mt-2' />
          </div>
        </div>
      )}
        ...
        <div className='mt-4 w-full flex items-center justify-end'>
          <div className='p-4 bg-zinc-800 rounded'>
            ...
            <div className='w-[100%] mt-3'>
              <button className='bg-zinc-100 rounded px-4 py-2 flex justify-center w-full font-semibold hover:bg-zinc-100' onClick={PlaceOrder}>
                Place your order
              </button>
            </div>
          </div>
